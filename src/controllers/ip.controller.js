...
export const getFilteredIp = async (req, res, next) => {
    try {
        const torIPs = await torService.fetchAllTorIPs();
        const blockedIPs = await BlockedIP.find({}, 'ip');
        const blockedSet = new Set(blockedIPs.map(ip => ip.ip));
        const filteredIPs = torIPs.filter(ip => !blockedSet.has(ip));
